...
def check_all_models():
    ...
    truth_values = {}
    table = list(itertools.product(['False', 'True'], repeat=len(identifiers)))
    for model in table:
        i=0
        for id in identifiers:
            truth_values[id] = model[i]
            i+=1

        kb_eval = evaluate_expression_set(kb, truth_values)
        infer_eval = evaluate_expression_set(infer, truth_values)
